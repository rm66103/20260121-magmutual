...
FastAPI backend application for querying user data from CSV files with semantic similarity search capabilities.
...
**Important**: Make sure you're in the project root directory when running the server.
...
**Note**: If you get a `ModuleNotFoundError: No module named 'api'`, make sure you're running the command from the project root directory (the directory containing the `api/` folder).
...
Semantic search for users by profession using vector similarity. Supports optional date filtering.
...
GET /users/search?profession=engineer&start_date=2023-01-01&end_date=2023-06-30
```
...
4. Date filters are applied to the semantic search results
5. Full user records are returned with similarity scores
